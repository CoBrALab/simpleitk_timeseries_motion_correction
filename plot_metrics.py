...
def main():
    parser = argparse.ArgumentParser(description="Compute and plot similarity metrics for 4D timeseries.")
    parser.add_argument("--compare", required=True, action='append', help="Pair of images: moving_4d,fixed_3d")
    ...
    for pair_str in args.compare:
        ...
        if len(parts) != 2:
            raise ValueError(f"Invalid compare argument '{pair_str}'. Must be 'moving_image,fixed_image'")
        ...
        if timepoints is None:
            ...
            print(f"Warning: {moving_path} has {num_timepoints} timepoints, expected {len(timepoints)}. Plotting may be mismatched.")
    ...
    metrics_to_plot = ['Mattes Mutual Information', 'Correlation', 'Mean Squares', 'Cosine Distance', 'Joint Histogram MI']
    
    for i, metric_name in enumerate(metrics_to_plot):
        ax = axes_flat[i]
        for label, history in all_results.items():
            values = history[metric_name]
            ax.plot(range(len(values)), values, marker='o', linestyle='-', markersize=2, label=label)
